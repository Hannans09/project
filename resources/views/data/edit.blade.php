            ...
            <form action="{{ route('data.update', $data->id) }}" method='post' enctype="multipart/form-data">
                ...
                <div class="relative z-0 w-full mb-6 group">
                    <select name='kriteria_id' id="kriteria_id"
                        ...
                        @foreach ($kriteria as $krt)
                            <option value="{{ $krt->id }}" {{ $data == 'kriteria_id' ? 'selected' : '' }}>
                                {{ $krt->nama }}</option>
                        @endforeach
                    </select>
                ...
                <div class="relative z-0 w-full mb-6 group">
                    <select name='alternatif_id' id="alternatif_id"
                        ...
                        @foreach ($alternatif as $alt)
                            <option value="{{ $alt->id }}" {{ $data == 'alternatif_id' ? 'selected' : '' }}>
                                {{ $alt->nama }}</option>
                        @endforeach
                    </select>
                ...
                <div class="relative z-0 w-full mb-6 group">
                    <input type="number" name='value' id="value"
                        class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                        placeholder=" " required value={{ $data->value }} />
                    <label for="value"
                ...
                <a href={{ route('data.index') }} type="button"
